...
def reply_message(event):
    ...
    if 'help' in event.message.text.lower():
        message = [TextSendMessage(text='以下是您可以使用的指令:\n警鈴\n開燈\n關閉\npir\ndisconnect'),
        StickerSendMessage(
            package_id = '446',
            sticker_id = '1989'
        )]
        ...
        while True:
            ...
            if i==0: #When output from motion sensor is LOW
                ...
                if current_time - last_message_time >= 10:  # Check if at least 10 seconds have passed
                    ...
                    line_bot_api.push_message(line_user_id, TextSendMessage(text='偵測到移動!!'))
            ...
            if True in matches:
                ...
                if face_count > 10:
                    ...
                    if current_time - last_email_time > 30:  # check if at least XX seconds have passed
                        ...
                        line_bot_api.push_message(line_user_id, TextSendMessage(text=currentname + '來囉'))
                        line_bot_api.push_message(line_user_id, StickerSendMessage(package_id=11537, sticker_id=52002738))
                        imgurl = upload_to_imgur('image.jpg', imgur_client_id)
                        line_bot_api.push_message(line_user_id, ImageSendMessage(original_content_url=imgurl, preview_image_url=imgurl))
                ...
                if current_time - last_email_time > 30 and unknown_count > 20:  # check if at least XX seconds have passed
                    ...
                    unknown_count = 0
                    last_email_time = current_time  # update the time the last email was sent
                    line_bot_api.push_message(line_user_id, TextSendMessage('有陌生人來囉'))
                    line_bot_api.push_message(line_user_id, StickerSendMessage(package_id=446, sticker_id=2011))
                    imgurl = upload_to_imgur('image.jpg', imgur_client_id)
                    line_bot_api.push_message(line_user_id, ImageSendMessage(original_content_url=imgurl, preview_image_url=imgurl))
